...
class PatreonAuthController < ApplicationController
  ...
  def start
    redirect_to "https://www.patreon.com/oauth2/authorize?response_type=code&client_id=#{ENV.fetch('PATREON_CLIENT_ID')}&redirect_uri=#{REDIRECT_URI}&state=#{current_podcast.id}"
  end
  ...
  def redirect
    ...
    patreon_user = PatreonUser.find_or_create_by!(patreon_user_id: user.id) do |patreon_user|
      ...
      patreon_user.user_data = user.as_json
    end
